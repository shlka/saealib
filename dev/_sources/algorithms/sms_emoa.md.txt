...
**Handling the reference point**: `HypervolumeComparator(reference_point=...)` lets you specify a fixed value; the default `None` computes it automatically per generation and per front.
The automatic computation uses a relative margin, "worst objective value + `margin` * (worst value − best value)" (default `margin=0.1`), a different formula from the paper's absolute offset, "worst objective value + 1.0" (Section 2.1.3).
