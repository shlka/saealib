...
コアAPIは、コンポーネント契約、グラフ、状態、コンパイル済み計画の公開ファサードです。 通常の拡張では `saealib.core` から名前を取得し、実装モジュールのパスを直接参照しません。
...
`Compiler` の公開 import 方針は現在整理中です。 そのため、このページでは `saealib.core` と `saealib.core.compiler` のどちらかを標準のimport pathとして断定しません。 コンパイラを使うコードでは、対象リリースの公開APIとリリースノートを確認してください。

グラフの構成値は`saealib.core.compiler`から参照できます。
