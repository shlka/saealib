...
`minimize()`では、アルゴリズム、サロゲートモデル、評価戦略に文字列またはインスタンスを指定できます。このページの例では問題固有の設定だけを追加し、それ以外はすべて既定値を使います。

:::{admonition} このページでできるようになること
:class: tip

このページを終えると、連続・整数・カテゴリ変数を`Problem`に定義し、型に対応した探索を実行できます。
:::

通常の連続変数だけなら[高レベルAPI](highlevel_api.md)を使い、独自の変数や演算子を追加する場合は[独自コンポーネント](custom_components.md)を参照してください。このページでは`Problem`に変数型を定義し、型に対応した組み込み演算子で探索を実行します。
...
各演算子の詳細は[Algorithm](../concepts/search_algorithms/algorithm.md)を参照してください。
