...
The original GP-UCB procedure finds the UCB maximizer at every iteration. saealib approximates this `argmax UCB` by selecting the top UCB candidates from the pool generated by `GA`.
`IndividualBasedStrategy.evaluation_ratio` also allows several selected candidates to receive true evaluations together.
`LowerConfidenceBound.kappa` is fixed and does not implement the $\sqrt{\beta_t}$ schedule that varies with the iteration count in the original method.
For a finite set $D$, the original method chooses the following $\beta_t$ to obtain a theoretical upper bound on cumulative regret.

$$\beta_t = 2 \log\left(\frac{|D|\, t^2 \pi^2}{6\delta}\right)$$

This expression increases the exploration weight logarithmically with $t$.
The original paper also reports that experiments may benefit from reducing the coefficient to one fifth through cross-validation because the theoretical schedule can be too exploratory when used unchanged.
saealib's default `kappa=2.0` is a fixed value equivalent to $\beta_t=4.0$.
A fixed `kappa` is not a schedule with the theoretical regret guarantee.
The crossover, mutation, parent-selection, and environmental-selection combination in the example is an saealib configuration choice, not part of the GP-UCB definition.
