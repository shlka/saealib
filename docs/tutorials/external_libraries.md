...
Currently, surrogate model adapters (scikit-learn, XGBoost, LightGBM, PyTorch), pymoo adapters (`Crossover`/`Mutation`/`Algorithm`/`Problem`), and DEAP adapters (`Crossover`/`Mutation`/`Algorithm`) are implemented. For users who would rather remove the runtime dependency on an external evolutionary-computation library, [Porting operators to native saealib code](#porting-operators-to-native-saealib-code) below covers rewriting a pymoo- or DEAP-style operator as a native `Crossover`/`Mutation` subclass instead.
