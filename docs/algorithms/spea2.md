...
flowchart TD
    ...
    subgraph GEN["One generation (DirectStrategy.step)"]
        direction TB
        ASK["GA.ask()<br/>Binary tournament selection →<br/>SBX crossover →<br/>Polynomial mutation to generate offspring<br/>(L5, L6)"] --> EVAL["True evaluation<br/>(no surrogate involved)"]
        EVAL --> COMB["GA.tell()<br/>Combine population and offspring<br/>into a single pool<br/>(L3)"]
        COMB --> FIT["SPEA2Comparator._fitness()<br/>spea2_fitness computes<br/>S(i) → R(i) → D(i) → F(i)<br/>(L2)"]
        FIT --> SORT["SPEA2Comparator.sort_population()<br/>F(i)&lt;1: spea2_truncation_order<br/>F(i)≥1: ascending F(i)<br/>Infeasible solutions: ascending cv<br/>(L3)"]
        SORT --> TRUNC["TruncationSelection<br/>Take top N individuals as Pt+1"]
    end
...
| Fitness computation | `SPEA2Comparator` (`sort_population` internally computes $S(i)$/$R(i)$/$D(i)$/$F(i)$ via `spea2_fitness`) | L2 |
| Environmental selection | `SPEA2Comparator.sort_population()` + `TruncationSelection()` (uses `spea2_truncation_order` for the non-dominated $F(i)<1$ block, ascending $F(i)$ for the dominated $F(i)\geq1$ block, and ascending `cv` for infeasible solutions) | L3 |
...
The original SPEA2 specifies population size $N$ and archive size $\bar N$ independently, but saealib's `GA.tell()` combines the population and offspring into one pool, so the same value is used for both.
