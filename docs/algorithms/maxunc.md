...
MaxUnc is better understood as the $\alpha \to \infty$ limit of the merit function described by Büche et al. than as an independent algorithm with its own named original formulation.
...
Because it does not use the predictive mean, it can be understood intuitively as favoring regions far from the training data. However, the shape of the uncertainty depends on the GP kernel, noise, and other factors, so MaxUnc does not simply identify the farthest point for every GP.
...
|---|---|---|
| Search algorithm | `GA` (the specific combination of crossover, mutation, and selection is not part of MaxUnc's definition) | L3 |
...
Because MaxUnc selects its candidate from the pool generated by `GA`, saealib approximates `argmax σ`.
`IndividualBasedStrategy.evaluation_ratio` also allows the top candidates to receive true evaluations together.
The crossover, mutation, parent-selection, and environmental-selection combination in the example is an saealib configuration choice, not part of the MaxUnc definition.
