...
サロゲートの精度に応じて`SurrogateManager`や`OptimizationStrategy`を動的に切り替える仕組みは、3つの独立した関心事の組み合わせでできている。
...
精度評価方法は`GlobalSurrogateManager`などの`accuracy_evaluator`引数として注入され、その結果は`surrogate_manager.last_accuracy`プロパティを経由して切り替え判断へ渡される。
この一連の仕組みは`iterate()`ループでの利用を前提としている。
高レベルAPI（`minimize`/`maximize`）は実行中のコンポーネント差し替え手段を提供しないため、これらのSwitcherは使えない。

## 精度指標：SurrogateAccuracyMetric

`SurrogateAccuracyMetric`が実装を要求するのは`name`（プロパティ）と`compute(y_true, y_pred) -> float`の2つである。
...
`AccuracyEvaluator`のコンストラクタで`metrics`を省略すると、この3指標全てが既定として使われる。

## 精度評価方法：AccuracyEvaluator

`AccuracyEvaluator`が実装を要求するのは`evaluate(surrogate, train_x, train_y) -> SurrogateAccuracy`の1つだけである。
...
`HeldOutAccuracyEvaluator`は、直近の真の評価点との比較用途{cite}`hanawa2025switching`で使う。

`SurrogateAccuracy`（`metrics: dict[str, float]`, `n_samples: int`）は、評価結果を保持する単純なコンテナで、`get(name, default=nan)`でメトリクス名から値を取り出す。

## 切り替え判断：AccuracyBasedSurrogateSwitcher

`AccuracyBasedSurrogateSwitcher`が実装を要求するのは`switch(accuracy: SurrogateAccuracy | None) -> T`の1つだけである。
`iterate()`ループの中で`optimizer.set_*()`と組み合わせて呼ぶ。
...
`ManagerSwitcher`/`StrategySwitcher`は、指定したメトリクスが閾値以上なら`primary`、そうでなければ`fallback`を返す単純な二値切り替えである。
`StrategySwitcher`の既定閾値`0.56`は、{cite}`hanawa2025switching`によるPS/IB-GB切り替えの実装である。

`GenCtrlSwitcher`は、二値切り替えではなく`gen_ctrl`を指数平滑化で連続的に調整する{cite}`repicky2017genctrl`。
公開属性`smoothed_error`に平滑化済みの誤差推定値を保持する状態を持つため、1回の`run`につき1つのインスタンスを使う。
`GenCtrlSwitcher.switch()`が返す`int`は、そのまま[GenerationBasedStrategy](strategies.md)の`gen_ctrl`引数に渡す想定である。

## 独自Switcherの実装方法

独自の切り替えロジックが必要な場合は、`AccuracyBasedSurrogateSwitcher`を継承して`switch()`だけを実装すればよい。
